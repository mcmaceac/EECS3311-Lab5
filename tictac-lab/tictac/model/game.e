	...
	play (player: STRING; position: INTEGER)
		...
		do
			if game_finished then
				err_message := "game is finished: => play again or start new game"
			else
				...
				if player /~ p1 then
					if player ~ p2 then
						err_message := "not this player's turn: => " + p1 + " plays next"
					else
						err_message := "no such player: => " + p1 + " plays next"
					end
				else if board[position] /~ '_' then
					err_message := "button already taken: => " + p1 + " plays next"
				else
